"""Density-band breakout detector — volume-acceptance variant (A/B).
...
trend), but the dense band is built from a **volume-at-price acceptance profile**
(:func:`src.indicators.density.volume_acceptance_profile`) instead of the
**time-at-price** profile. The band is therefore anchored to *where size actually
traded*, with the candle body weighted as acceptance and the wicks as rejection.

This exists to A/B the two density definitions on identical bars / exits. Keep the
trigger, causality and FireEvent semantics word-for-word the same as the
time-based sign so any backtest difference is attributable to the profile alone.

Causality: the profile is built over ``[t-window, t-1]`` (the breakout bar is
excluded so it cannot move the band it breaks); the "was inside" check uses the
prior close; the breakout is read off bar ``t``'s own close. No look-ahead.
"""
...
from src.indicators.density import VolTransform, value_area, volume_acceptance_profile
from src.signs.base import FireEvent, Sign


class DensityBreakoutVolSign(Sign):
    ...
    def __init__(
        ...
        meaning. The profile-specific knobs:

        Args:
            body_ratio: Weight on the candle body vs wicks when building the
                profile, in ``[0, 1]`` (see :func:`volume_acceptance_profile`).
                ``0.5`` = plain volume profile; ``> 0.5`` favours acceptance.
            vol_clip: Cap on a bar's normalised volume so one outsized print
                cannot define the whole band. ``None`` disables it. Default 8.0.
            vol_transform: Concave volume compression (``linear``/``sqrt``/``log``;
                see :data:`VolTransform`). Default ``"linear"`` (raw volume +
                ``vol_clip``) — in the smoke A/B it held the best OOS of the three;
                ``sqrt``/``log`` lower drawdown but did not improve OOS. Treat this
                as a knob for the rebenchmark to settle, not a fixed choice.

        Raises:
            ValueError: On out-of-range parameters (same bounds as the sibling
                sign, plus ``body_ratio`` in ``[0, 1]`` and ``vol_clip > 0``).
        """
        if window < 2:
        ...
        self.required_indicators = [f"density_vol_{window}_{n_bins}"]
    ...
    def _eval_end(
        ...
        centers, weights = volume_acceptance_profile(
            opens[sl],
            highs[sl],
            lows[sl],
            closes[sl],
            ...
            vol_transform=self.vol_transform,
        )

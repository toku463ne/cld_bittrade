"""A/B + walk-forward for the density_pullback stale-box (max_base_bars) gate.

The base-length diagnostic INVERTED the classical "longer base = stronger
breakout" prior: per-trade mean_r *declines* with the number of consecutive prior
closes accepted inside the rolling value-area band (0–1 bars: +0.0064; 32–63:
+0.0018; 64+: −0.0011 with DR 0.18). Good breakouts leave *young* pause-in-trend
boxes; breakouts from mature, long-defended value areas get faded. The
``max_base_bars`` knob skips the stale tail (85/559 trades carrying ~3% of PnL),
so this is a turnover/DD trim — the equity metric decides, per the contra-filter
precedent on vol_expansion_ride.

Protocol (anti-knife-edge): the win must be smooth across the threshold sweep and
hold across the fixed-config walk-forward folds; a single lucky cell = reject.

Run::

    uv run --env-file .env.bt python -m src.backtest.analysis.density_pullback_baselen_ab

Env knobs: ``DP_TF`` (1h), ``DP_PRODUCT`` (GMO_BTC_JPY), ``DP_FOLDS`` (6),
``DP_MAXBASE`` (comma list, default ``16,24,32,48,64``).
"""
...
def _run(
    bars: list[Bar], max_base: int | None, size: float, rate: float
) -> tuple[list[float], list[Trade]]:
    """Run one arm on a bar segment. Returns ``(equity_curve, trades)``."""
    strat = DensityPullbackStrategy(max_base_bars=max_base)
...
def _max_dd(eq: list[float], size: float, bars: list[Bar]) -> float:
    """Max drawdown of the mark-to-market equity path, as a fraction of notional."""
...
def run_ab() -> None:
    ...
    maxbases = [int(d) for d in os.environ.get("DP_MAXBASE", "16,24,32,48,64").split(",")]
    size = 0.001
    ...
    arms: list[tuple[str, int | None]] = [("baseline (shipped)", None)] + [
        (f"max_base={d}", d) for d in maxbases
    ]
